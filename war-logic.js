window.warRoom = function() {
    return {
        ...
        async fetchData() {
            this.loading = true;
            this.debugStatus = "Fetching...";

            // 1. Fetch Alliances
            const { data: dataA, error: errA } = await this.client.from('alliances').select('*');
            if (errA) console.error("Alliances Error:", errA);
            this.alliances = dataA || [];

            // 2. Fetch View (Players)
            const { data: dataP, error: errP } = await this.client.from('elite_forces_view').select('*');
            if (errP) console.error("View Error:", errP);
            this.players = (dataP || []).map(r => ({ 
                tag: r.tag, name: r.player_name, thp: Number(r.player_thp), faction: r.faction, server: r.server 
            }));

            // 3. Fetch Cities
            const { data: dataC } = await this.client.from('cities').select('*');
            this.cities = dataC || [];

            // 4. Fetch History
            const { data: dataH } = await this.client.from('history').select('*').order('timestamp', { ascending: false });
            this.history = dataH || [];

            // DEBUG ALERTS (Remove these once data shows up)
            if (this.alliances.length === 0) {
                this.debugStatus = "Warning: Alliance table empty or RLS active.";
            } else {
                this.debugStatus = `Success: ${this.alliances.length} Alliances found.`;
            }
        ...
        get factionData() {
            ...
            return this.alliances.map(a => {
                ...
                return { ...a, stash: Number(last.copper) + (rate * hoursSince), warStash: Number(last.copper) + (rate * (hoursSince + hoursTillWar)), rate: rate };
            });
        },
        ...
        updateClock() {
            ...
            if (cetTime < this.seasonStart) {
                ...
                const nxt = this.getNextWarTime();
                const dff = nxt - cetTime;
                this.phaseCountdown = `${Math.floor(dff/36e5)}h : ${Math.floor((dff%36e5)/6e4)}m : ${Math.floor((dff%6e4)/1e3)}s`;
                this.phaseAction = "Countdown to Battle Window";
            }
        },
