    ...
    return {
        ...
        updateClock() {
            ...
            if (cetTime < this.seasonStart) {
                ...
                this.currentPhase = curr.n; this.phaseAction = "Matchmaking status active.";
        ...
        async fetchData() {
            this.loading = true;
            try {
                const [resA, resP, resC, resH] = await Promise.all([
                ...
                this.alliances = (resA.data || []).map(a => ({
                    ...a,
                    tag: (a.tag || '').trim(),
                    faction: a.faction || 'Unassigned'
                }));

                this.players = (resP.data || []).map(p => ({
                    tag: (p.tag || '').trim(),
                    name: p.player_name || p.name || 'Unknown',
                    thp: Number(p.player_thp || p.thp || 0),
                    server: p.server
                }));

                this.history = (resH.data || []).map(h => ({
                    ...h,
                    tag: (h.tag || '').trim(),
                    copper: Number(h.copper || 0)
                }));

                this.cities = (resC.data || []).map(c => ({
                    ...c,
                    tag: (c.tag || '').trim()
                }));

                this.debugStatus = `Loaded ${this.alliances.length} Alliances`;
            } catch (e) {
                console.error("Sync Error:", e);
                this.debugStatus = "Supabase Error";
            }
            this.loading = false;
        },
        ...
        async saveCitiesToDB() {
            ...
            if (!c) return;
            this.debugStatus = "Writing...";
            const { error } = await this.client.from('cities').upsert({
                tag: c.tag.toLowerCase(),
                l1: c.l1, l2: c.l2, l3: c.l3, l4: c.l4, l5: c.l5, l6: c.l6
            });
            if (error) alert("DB Error: " + error.message);
            else { alert("Saved!"); this.modifiedTags = this.modifiedTags.filter(t => t !== this.editTag); await this.fetchData(); }
        },
